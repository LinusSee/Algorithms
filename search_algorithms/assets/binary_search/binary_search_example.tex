...
\begin{document}
	...
	\\
	Our goal is to find out, if the array contains the element 13.
	\begin{center}
		\begin{tikzpicture}
			...
			\filldraw [draw=black, fill=tocheck] (5, 0) rectangle (6, 1) node[midway]{13};
	...
	\noindent \\
	Our pivot is not equal to 13, so we haven't reached our goal yet. But since the list is sorted and 9 is less than 13, we know that both the pivot and
	everything on it's right side can not contain 13.
	\\
	This reduces the amount of elements we need to care about from 10 to 13.
	\begin{center}
		\begin{tikzpicture}
	...
	\noindent \\
	Now we have 5 elements left and want to check the rest of the array for 13. First we need to find the pivot element again. If we only looked at the
	...
	\newpage
	\noindent \\
	21 is still not equal to 13, but we since it is higher, we know that we can ignore everything on the right side. Again, this only works if the array is sorted.
	\begin{center}
		\begin{tikzpicture}
			\filldraw [draw=black, fill=checked] (0, 0) rectangle (1, 1) node[midway]{0};
			\filldraw [draw=black, fill=checked] (1, 0) rectangle (2, 1) node[midway]{1};
	...
	\begin{center}
		\begin{tikzpicture}
			...
			\filldraw [draw=black, fill=checked] (3, 0) rectangle (4, 1) node[midway]{6};
			\filldraw [draw=black, fill=checked] (4, 0) rectangle (5, 1) node[midway]{9};
			\filldraw [draw=black, fill=currentfocus] (5, 0) rectangle (6, 1) node[midway]{13};
			\filldraw [draw=black, fill=tocheck] (6, 0) rectangle (7, 1) node[midway]{15};
			\filldraw [draw=black, fill=checked] (7, 0) rectangle (8, 1) node[midway]{21};
			\filldraw [draw=black, fill=checked] (8, 0) rectangle (9, 1) node[midway]{22};
			\filldraw [draw=black, fill=checked] (9, 0) rectangle (10, 1) node[midway]{34};
